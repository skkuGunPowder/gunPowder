...
using DG.Tweening;
public class LobbyButtonPointer : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public void OnPointerEnter(PointerEventData eventData)
    {
        ...
        SelectedMask.transform.DOScaleX(1, Speed).SetEase(EaseType);
        if (NoText)
        {
            return;
        }
    ...
    {
        SelectedMask.transform.DOScaleX(0, Speed).SetEase(EaseType).OnComplete(() => SelectedMask.SetActive(false));
        if (NoText)
        {
            return;
        }
