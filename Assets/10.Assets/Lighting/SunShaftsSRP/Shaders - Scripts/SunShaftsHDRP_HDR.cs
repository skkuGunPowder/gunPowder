...
[Serializable]
[PostProcess(typeof(SunShaftsHDRP_HDRRenderer), PostProcessEvent.BeforeStack, "Custom/SunShaftsHDRP_HDR")]
public sealed class SunShaftsHDRP_HDR : PostProcessEffectSettings
{
    ...
    public FloatParameter resolutionDivider = new FloatParameter { value = 1.0f };
    public IntParameter blendChoice = new IntParameter { value = 0 };
...
public sealed class SunShaftsHDRP_HDRRenderer : PostProcessEffectRenderer<SunShaftsHDRP_HDR>
{
    ...
    public override void Render(PostProcessRenderContext context)
    {
        ...
        //int divider = 4;
        //if (settings.resolution == SunShaftsHDRP_HDR.SunShaftsResolution.Normal)
        //    divider = 2;
        // else if (settings.resolution == SunShaftsHDRP_HDR.SunShaftsResolution.High)
        //    divider = 1;

        if (settings.resolutionDivider.value <= 1)
        {
            settings.resolutionDivider.value = 1;
        }
        ...
        int rtW = (int)(context.width / settings.resolutionDivider.value); //source.width / divider;
        int rtH = (int)(context.width / settings.resolutionDivider.value); //source.height / divider;
        ...
        // context.command.BlitFullscreenTriangle(context.source, context.destination, sheetSHAFTS, (settings.screenBlendMode == SunShaftsHDRP_HDR.ShaftsScreenBlendMode.Screen) ? 0 : 4);

        if (settings.blendChoice.value == 0)
        {
            context.command.BlitFullscreenTriangle(context.source, context.destination, sheetSHAFTS, 0);
        }
        else
        {
            context.command.BlitFullscreenTriangle(context.source, context.destination, sheetSHAFTS, 4);
        }
