...
namespace Artngame.SKYMASTER
{
    ...
    [AddComponentMenu ("Image Effects/Rendering/Sun ShaftsSkyMaster HDR")]
    public class SunShaftsSkyMaster_HDR : PostEffectsBaseSkyMaster
    {
        ...
        void OnRenderImage (RenderTexture source, RenderTexture destination) {
            ...
            var format = GetComponent<Camera>().allowHDR ? RenderTextureFormat.DefaultHDR : RenderTextureFormat.DefaultHDR;
            ...
            if (!useDepthTexture) {
				...
				//var format= GetComponent<Camera>().allowHDR ? RenderTextureFormat.DefaultHDR: RenderTextureFormat.DefaultHDR; //v3.4.9
            ...
            for (int it2 = 0; it2 < radialBlurIterations; it2++ ) {
                ...
                lrColorB = RenderTexture.GetTemporary (rtW, rtH,  0, format);
                Graphics.Blit (lrDepthBuffer, lrColorB, sunShaftsMaterial, 1);
                RenderTexture.ReleaseTemporary (lrDepthBuffer);
                ofs = sunShaftBlurRadius * (((it2 * 2.0f + 1.0f) * 6.0f)) / 768.0f;
                sunShaftsMaterial.SetVector ("_BlurRadius4", new Vector4 (ofs, ofs, 0.0f, 0.0f) );

                lrDepthBuffer = RenderTexture.GetTemporary (rtW, rtH,  0, format);
            ...
            if (v.z >= 0.0f)
            {
                if (linearMode)
                {
                    sunShaftsMaterial.SetVector("_SunColor", new Vector4(sunColor.linear.r, sunColor.linear.g, sunColor.linear.b, sunColor.linear.a) * sunShaftIntensity);
                }
                else
                {
